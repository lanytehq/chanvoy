...
use chanvoy_mcp::with_request_meta;
...
#[tokio::test]
async fn mcp_stdio_cancelled_notification_closes_uds_and_admits_later_wait() {
    let env = TestEnv::new("per-041-notify-cancel").await;
    env.write_default_profile("agent-bravo-devlead", "org-lanytehq");
    let socket_path = env.socket_path();
    let listener = UnixListener::bind(&socket_path).expect("bind");
    let server = tokio::spawn(async move {

...
use chanvoy_core::{
    ...
    format_extended as format_host_extended, host_generation_match, is_mattermost_post_id,
    list_profiles, load_active_profile, load_profile, load_token, parse_after_channel_flag,
    parse_qualified_wait_selector, parse_time_window, pid_path_for_profile, provider_status_class,
    refuse_inbox_after, resolve_host_build_info, socket_path_for_profile, store_active_profile,
    store_profile, validate_wait_channels_params, AckResult, AttentionListResult,
...
enum DaemonCommand {
    ...
    Status,
    /// Exit 0 if this environment can restart the live daemon for a profile.
    ///
    /// PER-038A / FIX-2: ownership is **daemon-reported**, not env-vs-TOML
    /// string equality. Loads the profile token, runs whoami (start preflight),
    /// and compares to the live daemon's `status.mattermost_username`. Used by
    /// `make install-restart-daemons` so org-spanning seats whose ambient
    /// `LANYTE_MM_BOT_USERNAME` diverges from profile TOML bot naming still
    /// cycle their own daemon, while foreign seats stay left-running.
    /// Requires an explicit profile (`--profile` / `CHANVOY_PROFILE` / identity).
    Ownable,
}
...
struct WaitArgs {
    ...
    #[arg(required_unless_present_any = ["channels", "dm", "inbox"])]
    channel: Option<String>,
...
/// Whether this process environment may restart the live daemon for `profile`.
///
/// PER-038A FIX-2: ownable iff (1) profile token + start preflight whoami
/// succeeds and matches the persisted bot expectation, and (2) the live
/// daemon's reported `mattermost_username` equals that whoami. Never compares
/// `LANYTE_MM_BOT_USERNAME` to TOML — those strings follow divergent seat
/// naming conventions on shared hosts.
...
async fn handle_version(
    ...
    let (daemon_info, daemon_profile, daemon_error, daemon_username) =
        match probe_profile.as_deref() {
            Some(profile) => match status(profile).await {
                Ok(st) => (
                    st.binary,
                    Some(st.profile_name),
                    None,
                    Some(st.mattermost_username),
                ),
                Err(e) => (None, Some(profile.to_string()), Some(e.to_string()), None),
            },
            None => (None, None, None, None),
        };

    // FIX-1: only score generation when this environment owns the probed
    // daemon. A bare `active_profile` pointing at another seat must never
    // print a confident Generation: match that conceals a stale local daemon.
    let ownership = match daemon_profile.as_deref() {
    ...
    if json {
        ...
        if let Some(obj) = value.as_object_mut() {
            ...
            obj.insert(
                "daemon_username".into(),
                serde_json::to_value(&daemon_username)
                    .map_err(|e| CliError::Bootstrap(e.to_string()))?,
            );
            obj.insert(
                "generation_scored".into(),
                serde_json::Value::Bool(generation_scored),
            );
            obj.insert(
            ...
            if let Some(own) = &ownership {
                obj.insert(
                    "ownership".into(),
                    serde_json::to_value(own).map_err(|e| CliError::Bootstrap(e.to_string()))?,
                );
            }
    ...
    match (daemon_info.as_ref(), daemon_profile.as_deref()) {
        (Some(d), Some(profile)) => {
            ...
            if let Some(user) = daemon_username.as_deref() {
                println!("Daemon identity: {user}");
            }
            ...
            if generation_scored {
                match gen_match {
                    ...
                    Some(false) => {
                        ...
                        println!(
                            "     # or: make install-restart-daemons  (ownable profiles only)"
                        );
                    }
                    None => println!("Generation: unknown (incomplete pin on one side)"),
                }
            } else {
                // FIX-1 fail-closed: never a bare match/MISMATCH about a
                // daemon this environment does not own.
                let why = ownership
                    .as_ref()
                    .map(|o| o.reason.as_str())
                    .unwrap_or("ownership not determined");
                println!("Generation: not scored ({why})");
                println!(
                    "         probed profile `{profile}` is not restart-ownable in this environment"
                );
                println!(
                    "         target your seat: chanvoy --profile <your-profile> version --extended"
                );
            }
        }
        (None, Some(profile)) => {
        ...
        _ => {
            ...
            println!("Generation: not scored (no daemon profile resolved)");
            println!(
                "         target your seat: chanvoy --profile <your-profile> version --extended"
            );
        }
...
async fn execute(cli: Cli) -> Result<(), CliError> {
    ...
    let policy = match &cli.command {
        CommandSet::Daemon(DaemonCommand::Stop | DaemonCommand::Start | DaemonCommand::Ownable) => {
...
async fn handle_wait(profile: &str, json: bool, args: WaitArgs) -> Result<(), CliError> {
    if args.inbox {
        return handle_wait_inbox(profile, json, args).await;
    }
    if args.dm.is_some() {
...
fn refuse_inbox_selector_mix(args: &WaitArgs) -> Option<&'static str> {
    if args.dm.is_some() {
        return Some("wait --inbox refuses --dm; do not pass a channel id");
    }
    if args.channel.is_some() {
        return Some("wait --inbox refuses a positional channel; do not pass a channel id");
    }
    if !args.channels.is_empty() {
        return Some("wait --inbox refuses repeated --channel fan-in");
    }
    if !args.after_channels.is_empty() {
        return Some("wait --inbox refuses --after-channel");
    }
    if args.team.is_some() {
        return Some("wait --inbox refuses --team; DMs are not team channels");
    }
    None
}

async fn handle_wait_inbox(profile: &str, json: bool, args: WaitArgs) -> Result<(), CliError> {
    if let Some(message) = refuse_inbox_selector_mix(&args) {
        return exit_wait_hard(json, "inbox", "input", false, message);
    }
    if let Some(after) = args.after.as_deref() {
        if is_mattermost_post_id(after)
            || refuse_inbox_after(after).is_err() && is_mattermost_post_id(after)
        {
            return exit_wait_hard(json, "inbox", "input", false, POST_ID_NOT_INBOX_CURSOR);
        }
        if let Err(err) = refuse_inbox_after(after) {
            return exit_wait_hard(json, "inbox", "input", false, &err.to_string());
        }
    }
    if args.follow {
        return handle_wait_inbox_follow(profile, json, args).await;
    }
    let timeout_secs = match parse_time_window(&args.timeout, TimeWindowDefaultUnit::Minutes) {
    ...
    if !json {
        eprintln!("waiting for any DM to this bot (timeout: {timeout_secs}s)...");
    }
    let client = daemon_client(profile);
    let result = match client
        .wait_inbox_v1(WaitInboxV1Params {
            timeout_secs,
            contains: args.contains.clone(),
            pattern: args.pattern.clone(),
            after: args.after.clone(),
        ...
        .await
    {
        ...
        Err(DaemonError::Rpc {
            ...
            return exit_wait_hard(
                ...
                "capability",
                false,
                "the running daemon does not support wait --inbox (wait_inbox_v1); \
        ...
        Ok(result) => {
            if result.messages.is_empty() {
                return exit_wait_hard(
                    ...
                    "input",
                    false,
                    "wait --inbox returned success with no message payload",
                );
            }
            if !json {
...
async fn handle_wait_inbox_follow(
    profile: &str,
    json: bool,
    args: WaitArgs,
) -> Result<(), CliError> {
    ...
    if !json {
        eprintln!("following any DM to this bot (timeout: {timeout_secs}s)...");
    }
    let seen_wait_id = Arc::new(Mutex::new(None::<String>));
    let seen_cursor = Arc::new(Mutex::new(None::<String>));
    let outcome = {
    ...
    match outcome {
        Ok(result)
            if matches!(
                result.kind,
                chanvoy_core::WaitInboxFollowResultKind::Deadman { .. }
            ) =>
        {
            process::exit(1)
        }
        Ok(result)
            if matches!(
                result.kind,
                chanvoy_core::WaitInboxFollowResultKind::Replaced { .. }
            ) =>
        {
        ...
        Ok(_) => exit_wait_hard(
            ...
            "provider",
            false,
            "held inbox wait returned a non-terminal result",
        ),
        Err(DaemonError::Io(err)) => exit_wait_hard(json, "inbox", "sink", false, &err.to_string()),
        Err(DaemonError::Rpc {
            code: RPC_UNKNOWN_METHOD,
            ..
        }) => exit_wait_hard(
...
impl FollowSink {
    ...
    fn emit_json<T: serde::Serialize>(&mut self, event: &T) -> Result<(), std::io::Error> {
        match self {
            Self::File(file) => {
...
async fn handle_daemon(profile: &str, json: bool, command: DaemonCommand) -> Result<(), CliError> {
    match command {
        DaemonCommand::Ownable => {
            let report = probe_restart_ownership(profile).await;
            if json {
                println!(
                    "{}",
            ...
            } else if report.ownable {
                println!(
                    "ownable: yes (profile {profile}; cli={} daemon={})",
                    report.cli_username.as_deref().unwrap_or("?"),
                    report.daemon_username.as_deref().unwrap_or("?"),
                );
            } else {
                println!("ownable: no (profile {profile}; {})", report.reason);
            }
            if report.ownable {
                Ok(())
            } else {
...
impl HumanReadable for WaitInboxV1Result {
    fn to_human_string(&self) -> String {
        let header = format!(
            "inbox @{} ({}) post={} cursor={}",
            self.peer_username, self.dm_name, self.matched_post_id, self.next_inbox_cursor
        );
        let body = self
            .messages
            .iter()
